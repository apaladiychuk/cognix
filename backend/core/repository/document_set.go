...
type (
	DocumentSetRepository interface {
		FindByUser(ctx context.Context, userID uuid.UUID) ([]*model.DocumentSet, error)
...
func (r *documentSetRepository) FindByUser(ctx context.Context, userID uuid.UUID) ([]*model.DocumentSet, error) {
	documentSets := make([]*model.DocumentSet, 0)
	if err := r.db.WithContext(ctx).Model(&documentSets).
...
func (r *documentSetRepository) Update(ctx context.Context, set *model.DocumentSet) error {
	if _, err := r.db.WithContext(ctx).Model(set).Where("id = ?", set.ID).Update(); err != nil {
...
func (r *documentSetRepository) DeleteConnector(ctx context.Context, id int64) error {
	if _, err := r.db.WithContext(ctx).Model(&model.DocumentSetConnectorPair{}).
		Where("id = ?", id).Delete(); err != nil {
